...
$stdout = Join-Path $env:TEMP ("edge-redirect-manager-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("edge-redirect-manager-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5286"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $edge = Get-EdgePath
    $targets = @(
        @{ Url = "http://127.0.0.1:$port/"; File = "01-overview-proof.png"; Size = "1600,1240" },
        @{ Url = "http://127.0.0.1:$port/redirect-lane"; File = "02-redirect-lane-proof.png"; Size = "1600,1400" },
        @{ Url = "http://127.0.0.1:$port/migration-risk"; File = "03-migration-risk-proof.png"; Size = "1600,1180" },
